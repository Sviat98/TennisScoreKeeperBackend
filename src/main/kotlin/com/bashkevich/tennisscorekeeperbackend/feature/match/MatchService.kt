...
class MatchService(
    ...
    suspend fun addMatch(matchBody: MatchBody): ShortMatchDto {
        return dbQuery {
            ...
            validateBody(matchBody) {
                val firstPlayerId = matchBody.firstPlayer.id.toInt()
                val secondPlayerId = matchBody.secondPlayer.id.toInt()
    ...
    suspend fun updateServe(matchId: Int, serveBody: ServeBody) {
        return dbQuery {
            if (matchId == 0) throw BadRequestException("Incorrect id")
            val firstServePlayerId = serveBody.servingPlayerId.toInt()

            val matchEntity = matchRepository.getMatchById(matchId) ?: throw NotFoundException("No match found!")

            validateBody(serveBody) {
                if (firstServePlayerId in listOf(matchEntity.firstPlayer.id, matchEntity.secondPlayer.id))
                    "Serve player id is not in match" else ""
            }

            matchRepository.updateServe(matchId, firstServePlayerId)
    ...
    suspend fun updateScore(matchId: Int, changeScoreBody: ChangeScoreBody) {
        return dbQuery {
            if (matchId == 0) throw BadRequestException("Incorrect id")
            val scoringPlayerId = changeScoreBody.playerId.toInt()

            val matchEntity = matchRepository.getMatchById(matchId) ?: throw NotFoundException("No match found!")

            validateBody(changeScoreBody) {
                if (scoringPlayerId !in listOf(matchEntity.firstPlayer.id, matchEntity.secondPlayer.id))
    ...
    private fun findSetTemplate(matchEntity: MatchEntity, setNumber: Int, setsToWin: Int): SetTemplateEntity {
        ...
        val currentSetTemplate = if (isDecidingSet) {
            setTemplateRepository.getSetTemplateById(matchEntity.decidingSet) ?: throw NotFoundException("No deciding set template found in match!")
        } else {
            setTemplateRepository.getSetTemplateById(matchEntity.regularSet) ?: throw NotFoundException("No regular set template found in match!")
        }
